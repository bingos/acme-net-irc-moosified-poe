package Acme::Net::IRC::Moosified::POE::Event;
...
sub BUILDARGS {
  ...
  @$self{qw(nick user host)} = parse_user( $from );
  $self->{userhost} = join '@', @$self{qw(user host)};

  if ($self->{type} !~ /\D/) {
    $self->{type} = $class->trans($self->{type});
  } 
  else {
